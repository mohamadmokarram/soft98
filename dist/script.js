...
function mobileViewClickHandler() {
    mobileListItems.forEach(listItem => listItem.addEventListener("click", mobileListItemClickHandler.bind(listItem)));
...
function mobileListItemClickHandler() {
    ...
    const wasOpen = submenuItem.classList.contains("postransform") &&
...
function removeMobileClickHandlers() {
    mobileListItems.forEach(listItem => listItem.removeEventListener("click", mobileListItemClickHandler.bind(listItem)));
}
...
if (searchInput) {
    searchInput.addEventListener("input", () => {
        ...
        const filteredData = DATA.filter(post => (post.title.toLowerCase().includes(InputValue) &&
        ...
        if (filteredData.length > 0) {
            ...
            $(".main-content").innerHTML = `<p style="text-align: center;margin-top: 50px;">برنامه مورد نظر یافت نشد</p>`;
        }
    });
}
//==========================================================================
