...
let $ = (selector: string): Element | null => {
  return document.querySelector(selector);
};

const expandListArrowButton = $("ul.apps button span") as HTMLButtonElement;
const expandButton = $(".continue") as HTMLButtonElement;
const logo = $(".logo img") as HTMLImageElement;
const weatherIcon = $(".moon-icon>i") as HTMLElement;
const ul = $("#nav ul.menu") as HTMLUListElement;
const hamburgerMenuIcon = $("#nav span.menusign") as HTMLSpanElement;
const hamburgerCloseIcon = $("#nav span.close") as HTMLSpanElement;
const systemItem = $(".system-item") as HTMLLIElement;
const mobileItem = $(".mobile-item") as HTMLLIElement;
const softwareItem = $(".software-item") as HTMLLIElement;
const graphicItem = $(".graphic-item") as HTMLLIElement;
const mobileCloseButton = $("#nav span.close") as HTMLSpanElement;
const adverBox = $("div.ads") as HTMLDivElement;

const mobileListItems: HTMLLIElement[] = [
  systemItem,
  mobileItem,
  softwareItem,
  graphicItem,
...
const toggleClasses = (element: HTMLElement, ...classNames: string[]) => {
...
if (expandButton) {
  ...
  expandButton.onclick = function () {
    ...
    let appsList = document.querySelector("ul.apps")! as HTMLUListElement;

    if (!appsMenuIsOpen) {
      appsList.style.setProperty("max-height", `${appsList.scrollHeight}px`);
      expandListArrowButton.innerHTML = "expand_less";
      appsMenuIsOpen = !appsMenuIsOpen;
    } else {
      appsList.style.setProperty("max-height", initialHeight);
...
function closeSubmenuHandler(): void {
...
function mobileListItemClickHandler(this: HTMLElement) {
  const submenuItem = this.querySelector(".submenu") as HTMLUListElement;
  const wasOpen =
...
if (screen.width < 811) {
  let lastul = document.querySelector(
    "div.submenu ul.col-3"
  ) as HTMLUListElement;
...
//============================================================================
let androidButton = document.querySelector("h3.android") as HTMLButtonElement;
let softwareIcon = document.querySelector(".pc-icon") as HTMLHeadingElement;
let androidUl = document.querySelector(".android-ul") as HTMLUListElement;
let softUl = document.querySelector(".soft-ul") as HTMLUListElement;
...
let filteredData;
function filterArray(category: string) {
  if (category === "انتخاب") {
    creatPosts(DATA);
  } else {
...
function creatPosts(filteredArray: Post[]) {
  let row = "";
  ...
  if ($(".main-content")) {
    const mainContent = $(".main-content") as HTMLDivElement;
...
let searchInput = $('input[type="search"]') as HTMLInputElement;
